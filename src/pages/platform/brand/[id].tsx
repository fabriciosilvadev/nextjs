...
export default function FormUpdate() {
  ...
  const getItem = async () => {
    const request = await apiService.get("/api/brand/view/" + id);
  ...
  const save = async (data: BrandUpdate) => {
    const request = apiService.put("/api/brand/update/" + id, data);

    if ((await request).status) {
      alert("Atualização realizada com sucesso!");
      router.push("/platform/brand/list");
    } else {
      alert("Houve um erro ao atualizar uma nova marca");
    }
  };
